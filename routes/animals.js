...
// Buscar animais (agora aceitando filtros corretamente)
// Buscar animais com filtros
router.get("/", authMiddleware, async (req, res) => {
  try {
    const { nome, especie, sexo, idade, porte } = req.query;

    const filtro = { ong: req.user.id }; // <-- só traz os animais da ONG logada
...
router.put("/:id", authMiddleware, upload.array("fotos"), async (req, res) => {
  try {
    ...
    if (req.files.length > 0) {
      atualizacao.fotos = req.files.map((file) => file.filename); 
    }
